...
const listMine = asyncHandler(async (req, res) => {
  res.json(await medicalRecordService.listOwnMedicalRecords(req.user));
});
...
const listByPatient = asyncHandler(async (req, res) => {
  ...
  res.json(await medicalRecordService.getPatientMedicalHistory(req.user, params.patientId));
});

const createConsultation = asyncHandler(async (req, res) => {
  const payload = consultationSchema.parse(req.body);
  const result = await medicalRecordService.createConsultation(req.user, payload);
  res.status(201).json(result);
});

const downloadPrescriptionPdf = asyncHandler(async (req, res) => {
  const params = z.object({ id: z.coerce.number().int().positive() }).parse(req.params);
  const pdf = await medicalRecordService.buildPrescriptionPdf(req.user, params.id);
